...
local screenGui=Instance.new("ScreenGui",p:WaitForChild("PlayerGui"))
local timerFrame=Instance.new("Frame",screenGui)
timerFrame.Size=UDim2.new(0,150,0,50)
timerFrame.Position=UDim2.new(0.5,-75,0.5,-25)
...
timerLabel.Text="Time:"..math.floor(timeElapsed).."s"
if timeElapsed>=timeLimit then break end
end
if timeElapsed>=timeLimit then break end
end
...
gui.InputBegan:Connect(function(input)if input.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true dragStart=input.Position startPos=gui.Position input.Changed:Connect(function()if input.UserInputState==Enum.UserInputState.End then dragging=false end end)end end)
